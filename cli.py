...
manager = tv.Manager(config.SHOWS.data, config.SEEN.data)
...
class EpisodesCmd(cmd.Cmd):
    ...
    def complete_see(self, text, line, start_index, end_index):
        return manager.complete(text)
    
    def do_ls(self, line):
        if not line:
            line = "*"
            
        for eurl in manager.pending.filter(line):
    ...
    def do_save(self, line):
        config.SEEN.data = manager.seen.to_str_list()
        config.SEEN.save()
